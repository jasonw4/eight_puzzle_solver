...
class State:
    ...
    def __init__(self, board, predecessor, move):
        """ initializes the state class with four attributes: board, predecessor, move, and num_moves
        """
        self.board = board
    ...
    def generate_successors(self):
        ...
        for x in MOVES:
            ...
            if b.move_blank(x) == True:
                newstate = State(b, self, x)
                successors += [newstate]
        return successors
    
    def print_moves_to(self):
        """ that prints the sequence of moves that lead from the initial state to the called State object
        """
        if self.predecessor == None:
            print('initial state:')
            print(self.board)
        else:
            recprint = self.predecessor.print_moves_to()
